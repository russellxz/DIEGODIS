...
// Only advertise what this build can actually decompress. Zstandard gives the
// best ratio of the three but is only built into newer Node releases.
const ACCEPT_ENCODING = (Zlib.createZstdDecompress ? "zstd,br,gzip,deflate" : "br,gzip,deflate");

/**
 * Reusing connections avoids a TLS handshake per request, which dominates
 * latency on small API calls. Shared across clients, like Node's own global
 * agent, but with keep-alive on regardless of the Node version in use
 */
const defaultAgent = new HTTPS.Agent({
  keepAlive: true,
  keepAliveMsecs: 15000,
  // Reuse the most recently active socket so idle ones are freed instead of
  // being kept barely alive by round-robin
  scheduling: "lifo",
  timeout: 30000,
});
...
class RequestHandler {
  constructor(client, options) {
    ...
    this.options = options = Object.assign({
      agent: client.options.agent || defaultAgent,
      ...
      const actualCall = (cb) => {
        ...
        req.once("response", (resp) => {
          ...
          if (resp.headers["content-encoding"]) {
            ...
            if (encoding.includes("zstd") && Zlib.createZstdDecompress) {
              _respStream = resp.pipe(Zlib.createZstdDecompress());
            } else if (encoding.includes("br")) {
              _respStream = resp.pipe(Zlib.createBrotliDecompress());
