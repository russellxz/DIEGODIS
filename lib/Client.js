...
class Client extends EventEmitter {
   ...
   * @arg {Boolean | String} [options.compress=false] The transport compression to request for WebSocket data. `true` and `"zlib-stream"` use zlib (requires zlib-sync or pako), `"zstd-stream"` uses Zstandard (requires Node 22.15.0 or newer, and compresses better), `false` disables compression
  ...
  constructor(token, options) {
    ...
    if (this.options.compress) {
      // Transport compression. "zlib-stream" needs zlib-sync or pako, while
      // "zstd-stream" uses Node's built-in Zstandard support and compresses
      // noticeably better for the same CPU
      const transport = this.options.compress === true ? "zlib-stream" : this.options.compress;
      if (transport !== "zlib-stream" && transport !== "zstd-stream") {
        throw new TypeError(`Invalid compress option: ${this.options.compress}. Use false, true, "zlib-stream" or "zstd-stream"`);
      }
      if (typeof window !== "undefined") {
        this.options.compress = false; // zlib does not like Blobs
      } else if (transport === "zstd-stream" && !Zlib.createZstdDecompress) {
        throw new TypeError("The \"zstd-stream\" compress option requires Node 22.15.0 or newer");
      } else {
        this.options.compress = transport;
      }
    }
  ...
  async connect() {
    ...
    try {
      ...
      if (this.options.compress) {
        this.gatewayURL += `&compress=${this.options.compress}`;
      }
   ...
   * @arg {String} [options.encryptionMode] Force a voice encryption mode instead of negotiating the best available one. Either "aead_aes256_gcm_rtpsize" or "aead_xchacha20_poly1305_rtpsize"
