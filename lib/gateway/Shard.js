...
class Shard extends EventEmitter {
  ...
  initializeWS() {
    ...
    this._destroyDecompressor();
    if (this.client.options.compress === "zstd-stream") {
      this.emit("debug", "Initializing zstd-based compression");
      this._initStreamDecompressor(Zlib.createZstdDecompress());
      ...
      if (ZlibSync) {
        ...
        // Node's own inflate handles the same continuous stream, so zlib-sync
        // is a performance option rather than a requirement
        this.emit("debug", "Initializing zlib-based compression");
        this._initStreamDecompressor(Zlib.createInflate());
      }
    }
    if (this.sessionID) {
      if (!this.resumeURL) {
      ...
      case "READY": {
        ...
        if (packet.t === "RESUMED") {
          ...
          if (this.client.options.compress) {
            this.resumeURL += `&compress=${this.client.options.compress}`;
          }
        }
  ...
  _destroyDecompressor() {
    if (this._decompressor) {
      this._decompressor.removeAllListeners();
      this._decompressor.destroy();
      this._decompressor = null;
      this._decompressChunks = null;
    }
    this._zlibSync = null;
  }

  _initStreamDecompressor(stream) {
    this._decompressChunks = [];
    this._decompressor = stream;
    stream.on("data", (chunk) => this._decompressChunks.push(chunk));
    stream.on("error", (err) => this.emit("error", err, this.id));
  }
  ...
  _onWSMessage(data) {
    try {
      ...
      if (this._decompressor) {
        // Discord flushes the stream after every message, so one write plus a
        // flush yields exactly one payload. The stream emits in write order, so
        // packets still reach onPacket in the order they arrived
        this._decompressor.write(data);
        this._decompressor.flush(() => {
          if (!this._decompressChunks || this._decompressChunks.length === 0) {
            return;
          }
          const decompressed = this._decompressChunks.length === 1 ? this._decompressChunks[0] : Buffer.concat(this._decompressChunks);
          this._decompressChunks.length = 0;
          try {
            this.onPacket(Erlpack ? Erlpack.unpack(decompressed) : JSON.parse(decompressed.toString()));
          } catch (err) {
            this.emit("error", err, this.id);
          }
        });
